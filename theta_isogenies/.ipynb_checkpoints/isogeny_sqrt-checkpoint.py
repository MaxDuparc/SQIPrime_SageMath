...
class ThetaIsogeny4(ThetaIsogeny):
    ...
    def __init__(self, domain, T1_4, T2_4, zeta, hadamard=(None, True)):
        self._zeta = zeta
    ...
    def _compute_codomain(self, T1, T2):
        ...
        A = ZZ(1)
        B = sqrt_Fp4(BB * AA_inv, self._zeta, True)
        C = sqrt_Fp4(CC * AA_inv, self._zeta, True)
        D = xCD * B /(xAB * C)
...
class ThetaIsogeny2(ThetaIsogeny4):
    ...
    def __init__(self, domain, zeta,T1=None, T2=None, hadamard=(None, True)):
        self._zeta = zeta
        super().__init__(domain, T1, T2, zeta, hadamard=hadamard)
    ...
    def _compute_codomain(self, T1, T2):
        ...
        B = sqrt_Fp4(BB * AA_inv, self._zeta, True)
        C = sqrt_Fp4(CC * AA_inv,self._zeta, True)
        D = sqrt_Fp4(DD * AA_inv,self._zeta, True)
